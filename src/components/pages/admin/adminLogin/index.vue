      ...
      >
        <div class="max-w-xl lg:max-w-3xl">
          ...
          <h1 class="mt-6 text-2xl font-bold text-gray-900 sm:text-3xl md:text-4xl">
            Welcome back Admin , this is your Dashboard!
          </h1>
  ...
  setup() {
    ...
    const loginUser = async () => {
      try {
        ...
        const currUser = resp.user

        if (currUser.uid === 'SeFPNc7VwPaNYhgW4vgK4TyaPGL2') {
          // Admin user: Redirect to /adminDashboard
          successMessage.value = 'Admin Login successful. Redirecting...'

          setTimeout(() => {
            localStorage.setItem('ActiveUser', currUser.uid)
            router.push('/adminDashboard')
          }, 2000) // 2 seconds delay
        } else {
          // Non-admin user: Redirect to /dashboard
          successMessage.value = 'User Login successful. Redirecting...'
